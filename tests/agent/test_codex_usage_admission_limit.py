...
def _entry(
    credential_id: str = "43c798",
    *,
    token: str = "vicky-token",
    priority: int = 0,
) -> PooledCredential:
...
def test_policy_window_mixed_durations_uses_complete_reset_fallback():
    from agent.credential_pool import _select_codex_policy_window
    ...
    assert selected is longer_without_duration
...
def test_specific_lease_refuses_credential_at_threshold(monkeypatch):
    ...
    assert pool.acquire_lease("43c798") is None
...
def test_threshold_change_reuses_fresh_usage_but_recomputes_admission(monkeypatch):
    ...
    monkeypatch.setattr(
        credential_pool,
        "get_pool_usage_limits",
        lambda provider: {"43c798": threshold[0]},
    )
    monkeypatch.setattr(
        account_usage,
        "fetch_account_usage",
...
def test_selection_reads_usage_limit_config_once_outside_entry_filter(monkeypatch):
    ...
    monkeypatch.setattr(
        credential_pool,
        "get_pool_usage_limits",
        lambda provider: config_calls.append(provider) or {"43c798": 80.0},
    )
    monkeypatch.setattr(
        account_usage,
        "fetch_account_usage",
    ...
    assert _select_id(CredentialPool("openai-codex", [_entry()])) == "43c798"
...
def test_auxiliary_codex_does_not_fall_back_after_pool_denial(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: singleton_calls.append(True) or "vicky-token",
    )
    monkeypatch.setattr(
        auxiliary_client,
...
def test_cold_explicit_matching_pool_token_is_probed_before_admission(monkeypatch):
    ...
    assert (
        credential_pool.usage_admission_credential_denied("openai-codex", "vicky-token")
        is True
    )
    assert (
        credential_pool.usage_admission_credential_denied(
...
def test_auxiliary_cache_hit_rejects_explicit_policy_denied_codex_token(
    ...
    monkeypatch.setattr(
        auxiliary_client,
        "usage_admission_credential_denied",
        lambda provider, token: provider == "openai-codex" and token == "vicky-token",
    )
