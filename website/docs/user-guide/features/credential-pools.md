...
are cached for five minutes; failed or malformed probes fail open and are
retried after 30 seconds. A three-second request timeout bounds cold-selection
latency. Eligibility returns automatically after a fresh probe reports usage
below the threshold. No policy state or access token is written to `auth.json`.
